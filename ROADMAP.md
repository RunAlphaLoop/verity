...
- [x] Identity: canonical principal registry (`/v1/admin/principals`), group management API, Drive connector principal crosswalk. *As built: Google Admin SDK directory sync ships as `verity_ingest.connectors.gdirectory` — full users/groups/members reconcile diffed against a local snapshot, direct (nested) membership edges into the group API, removals one tuple at a time via the tombstoning DELETE; **fixture-verified per §6c** — nested-closure cycle safety and suspended-user deprovisioning are tested against Google's documented API shapes, but it has not run against a live Google org: live-org validation awaits a customer Workspace admin credential (DWD service account + admin subject). The Directory API has no incremental sync token, so group-membership freshness = the reconcile interval; `users.watch` push is v0.2.*
