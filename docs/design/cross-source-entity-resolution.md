...
| `min_independent_keys` | smallint | **default 2** — a single MEDIUM key (e.g. shared domain) may not auto-merge alone (grafted from P0) |
...
| `tau_nil` | real | Tier-3 NIL threshold (§5) |
| `margin_delta` | real | Tier-3 top1−top2 abstain margin (§5) |
...
2. **`min_independent_keys` default = 2** — does the domain-only auto-merge become too conservative for tenants with clean single-CRM data? Should the default be per-`key_kind` (external_id=1, domain=2)?
3. **Incremental-fold cluster-join policy** — exact size floor / tier bar above which "join two existing components" must route to review rather than auto-join. Needs a measured default.
...
6. **`tau_nil` / `margin_delta` starting values** — bootstrap from the knowledge-merge judge's operating point, or measure fresh on a tenant-catalog EL benchmark before publishing metric #5?
